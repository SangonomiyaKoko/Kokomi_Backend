...
_data = FileUtils.load_json(
    ...
    default={'mode': 'default', 'points': []}
)
PROXY_CONFIG = (_data['mode'], _data['points'])
...
CREATE_SQL = FileUtils.load_sql(
    fp=INIT_DIR / 'sqlite/version.sql'
)

# 榜单 Redis 键的前缀，与旧版 cache 的 leaderboard 区分（暂时分离，不覆盖旧数据）
LEADERBOARD_PREFIX = 'leaderboard_v2'