...
class ClanSeasonUpdater:
    ...
    @classmethod
    def main(
        cls, run_ctx: RunContext, clan_id: int
    ) -> None:
        """执行单个公会的完整更新流程"""
        ...
        try:
            record_inc, discard_inc = cls._plan_record(ctx)
            ...
            record_inc = discard_inc = 0

        # 提交数据库写入并刷新排行榜缓存
        cls._commit(run_ctx, ctx)
        run_ctx.redis_client.zadd(
            RedisKeys.clan_zrank(),
            {str(ctx.clan_id): ctx.stats.clan_rating}
        )
        # 仅在本轮数据确实入库后才计入统计
        run_ctx.record_match += record_inc
        run_ctx.discard_match += discard_inc
    ...
    def _fetch_stats(
        ...
        try:
            ...
            # 加载本地缓存的旧队伍数据
            with mysql_read_only(run_ctx.mysql_connection, ctx.clan_id) as cursor:
    ...
    def _plan_record(
        ...
        返回 (record_inc, discard_inc) 由调用方在入库成功后计入统计
        """
        # 计算两份数据之间的场次差值
        ...
        if ctx.cache.season != ctx.season_id:
            # SEASON ID 不一致则将最新数据中所有场次直接抛弃
            return 0, battles_diff
    ...
    @staticmethod
    def _commit(run_ctx: RunContext, ctx: UpdateContext) -> None:
        """提交本次更新"""
        # 先写 MySQL 确保即使 SQLite 数据库写入失败不影响主数据库数据的完整
        with mysql_transaction(run_ctx.mysql_connection, ctx.clan_id) as cursor:
        ...
        with sqlite_transaction(ctx.season_id) as cursor:
