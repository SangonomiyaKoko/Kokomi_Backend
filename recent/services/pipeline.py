...
class UserDataProcessor:
    ...
    async def main(
        ...
        # 按需构建请求目标
        targets = EndpointRegistry.build_targets(
            account_id=ctx.account_id,
            access_token=ctx.access_token,
            fetch_modes=ctx.fetch_modes
        )

        # 并发请求
        fetch_result = await APIRequester.fetch(
        ...
        if not fetch_result:
            return UpdateResult.failed(FailedReason.OBTAIN_DATA_FAILED)
        ...
        if len(ctx.fetch_modes) == 0:
            return UpdateResult.skipped(SkippedReason.NO_FETCH_MODES)

        # 校验 API 返回数据格式
        pre_result = cls._handle_validation_result(
        ...
        # 解析并挂载领域模型
        cls._parse_response(ctx, fetch_result, update_timestamp)

        # 校验数据解析结果的结构完整性
        post_result = cls._handle_validation_result(
    ...
    def _handle_validation_result(
        ...
        """统一处理效验结果，返回不为 None 则中断"""
        if result.is_failed:
            return UpdateResult.failed(result.reason)
        
        if result.is_skipped:
            return UpdateResult.skipped(result.reason)
    ...
    @staticmethod
    def _parse_response(ctx: UpdateContext, fr: FetchResult, updated_at: int) -> None:
        """解析返回的用户总体数据和各模式的详细数据"""
        # 解析用户总体数据流程
        ...
        if 'basic' not in statistics:
            ...
            ship_collection = {
                mode: ShipDataCollection() for mode in ctx.fetch_modes
            }
        ...
        if REGION == 'ru':
            ...
            clan_statistics = {
                ...
                'frags': (
                    solo_data.get('frags', 0) 
                    + div_data.get('frags', 0)
                ),
                'original_exp': (
