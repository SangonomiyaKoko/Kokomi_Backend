    ...
    def _write_index_data(cursor, ctx: UpdateContext) -> None:
        ...
        for mode in ctx.fetch_modes:
            ...
            if collection is None:
                continue
            for ship_id, ship_data in collection.ships.items():
    ...
    def _write_latest_index(cursor, ctx: UpdateContext, indices: dict) -> None:
        ...
        for mode in ctx.fetch_modes:
            ...
            if collection is None:
                continue
            for ship_id, ship_data in collection.ships.items():
                entries.setdefault(ship_id, {})[mode] = (
                    ship_data.battles, ctx.yesterday_date
                )

        insert_params = [
            ShipLatestIndexParams(
                ship_id=ship_id,
                pvp_battles=mode_map.get(BattleMode.PVP, (0, None))[0],
                rank_battles=mode_map.get(BattleMode.RANK, (0, None))[0],
                clan_battles=mode_map.get(BattleMode.CLAN, (0, None))[0],
                pvp_index=mode_map.get(BattleMode.PVP, (0, None))[1],
                rank_index=mode_map.get(BattleMode.RANK, (0, None))[1],
                clan_index=mode_map.get(BattleMode.CLAN, (0, None))[1],
            )
