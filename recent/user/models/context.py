...
from .ship_data import ShipDataCollection, ModeBattleStats
from .user import UserRecord, UserStats
from .mode import UpdateStrategy, BattleMode
...
class UpdateContext:
    ...
    # 用户数据库信息
    date_list: list[int] = field(init=False, default_factory=list)
    daily_summary: dict[int, DailySummary] = field(init=False, default_factory=dict)
    ship_cache: ShipCache = field(init=False, default_factory=ShipCache)

    # 数据接口请求结果 & 更新策略
    update_strategy: UpdateStrategy
    fetch_modes: set[BattleMode] = field(init=False, default_factory=set)
    mode_data: dict[BattleMode, ModeBattleStats] = field(init=False, default_factory=dict)
    ship_data: dict[BattleMode, ShipDataCollection] = field(init=False, default_factory=dict)
