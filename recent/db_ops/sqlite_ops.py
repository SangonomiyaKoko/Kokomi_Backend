...
@contextmanager
def sqlite_read_only(account_id: int) -> Iterator[Cursor]:
    """SQLite 上下文管理器，仅读取"""
    ...
    try:
        ...
        logger.error(f'{account_id} | Database read error: {type(e).__name__}')
        raise
    finally:
        conn.close()
...
@contextmanager
def sqlite_transaction(account_id: int) -> Iterator[Cursor]:
    """SQLite 自动事务上下文管理器"""
...
def ensure_database(account_id: int) -> bool:
    ...
    try:
        ...
        if db_path.exists():
            ...
            logger.warning(f"Corrupted file deleted: {db_path}")
        return False
    else:
        conn.commit()
        conn.close()
